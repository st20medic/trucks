...
import { z } from 'zod';
import { X, Truck, Save, Upload, Trash2, Plus, Minus } from 'lucide-react';
...
const addTruckSchema = z.object({
  ...
  status: z.enum(['in-service', 'out-of-service']).default('in-service'),
  outOfServiceReason: z.string().optional(),
  tireSize: z.string().default('Not specified'),
  tireConfiguration: z.enum(['dual-rear', 'single-rear']).default('single-rear'),
...
interface AddTruckModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: AddTruckFormData & { equipment: Equipment[] }) => Promise<void>;
}

const AddTruckModal: React.FC<AddTruckModalProps> = ({ isOpen, onClose, onSubmit }) => {
  const { user } = useAuth();
  ...
  const handleFormSubmit = async (data: AddTruckFormData) => {
    if (!user) return;
    ...
    try {
      ...
      setSuccessMessage('Error adding truck. Please try again.');
    } finally {
      setLoading(false);
    }
  };
